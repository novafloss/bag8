...
def test_develop(local_path, slave_id):
    ...
    interactive = '--interactive' if slave_id == 'default' \
        else '--no-interactive'

    # new instance
    proc = call(['bag8', 'develop', 'busybox', '-c', 'echo "first run"',
                 '-p', slave_id, interactive])
...
docker exec {it}{prefix}_busybox_1 echo "first run"
"first run"
""".strip().format(**{
        'it': '-it ' if slave_id == 'default' else '',
...
""".strip().format(**{
    ...
    assert proc.returncode == 0, err
    assert out.strip() == """
docker exec {it}{prefix}_busybox_1 echo "second run"
"second run"
""".strip().format(**{
        'it': '-it ' if slave_id == 'default' else '',
        'prefix': slave_id,
...
{prefix}_busybox_1
docker exec {it}{prefix}_busybox_1 echo "third run"
"third run"
""".strip().format(**{
        'it': '-it ' if slave_id == 'default' else '',
        'prefix': slave_id,
